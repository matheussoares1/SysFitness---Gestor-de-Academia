...
# Criando janela
janela = Tk()
janela.title("SysFitness - Beta Version 1.0.0")
janela.geometry('850x620')
janela.configure(background=co1)
janela.resizable(width=FALSE, height=FALSE)


icone = PhotoImage(file = 'SysFitness_remove_Background.png')
...
def cadastrar():
	...
	def novo_cliente():
		...
		nome = enome_cliente.get()
		cpf = ecpf.get()
		telefone = etelefone.get()
		endereco = eendereco.get()
		data_nasc = edata_nascimento.get()
		data_inicio_cliente = edata_inicio_clienter.get()
		personal = epersonal_nome.get()
		turma = eturmas_nome.get()



		lista = [nome, endereco, cpf, telefone, data_nasc, data_inicio_cliente, personal, turma]
		...
		enome_cliente.delete(0,END)
		ecpf.delete(0,END)
		etelefone.delete(0,END)
		eendereco.delete(0,END)
		edata_nascimento.delete(0,END)
		edata_inicio_clienter.delete(0,END)
		epersonal_nome.delete(0,END)
		eturmas_nome.delete(0,END)
	...
	def update_cliente():
		try:
			...
			enome_cliente.insert(0, tree_lista[1])
			ecpf.insert(0, tree_lista[2])
			etelefone.insert(0, tree_lista[3])
			edata_nascimento.insert(0, tree_lista[4])
			edata_inicio_clienter.insert(0, tree_lista[5])
			epersonal_nome.insert(0, tree_lista[6])
			eturmas_nome.insert(0, tree_lista[7])
	...
	botao_salvarcliente = Button(frame_detalhes,command=novo_cliente, anchor=CENTER, text="Salvar".upper(), width=20, overrelief=RIDGE, font=('Ivy 7 bold'), bg=co3, fg=co1)
	botao_salvarcliente.place(x=630, y=120)


	botao_atualizarcliente = Button(frame_detalhes,command=update_cliente, anchor=CENTER, text="Pesquisar".upper(), width=20, overrelief=RIDGE, font=('Ivy 7 bold'), bg=co6, fg=co1)
	botao_atualizarcliente.place(x=630, y=145)


	botao_deletarcliente = Button(frame_detalhes,command=delete_cliente, anchor=CENTER, text="Deletar".upper(), width=20, overrelief=RIDGE, font=('Ivy 7 bold'), bg=co7, fg=co1)
